...
arg_4=50000
arg_5=$analysis_dir/$chr"_target_out_bufferRegion"$arg_4".txt"
arg_6=$analysis_dir/$chr"_query_out_bufferRegion"$arg_4".txt"
arg_7=$analysis_dir/$chr"_window_query_out_bufferRegion"$arg_4".txt"
arg_8=$analysis_dir/$chr"_toMaskRegions.bed" ### masked regions in bed format 
arg_9=$analysis_dir/$chr"_identityMapped.bed" ### regions that are on the identity line in bed format 
python $scripts_dir/generate_masks.py $arg_1 $arg_2 $arg_3 $arg_4 $arg_5 $arg_6 $arg_7 $arg_8 $arg_9 $chr
...
sort -n -k 2,2 $analysis_dir/$chr"_toMaskRegions.bed" > $analysis_dir/$chr"_toMaskRegions_sorted.bed"
sort -n -k 2,2 $analysis_dir/$chr"_identityMapped.bed" > $analysis_dir/$chr"_identityMapped_sorted.bed"

# B. Use bedtools merge

bedtools merge -i $analysis_dir/$chr"_toMaskRegions_sorted.bed" > $analysis_dir/$chr"_toMaskRegions_sorted_merged.bed"
bedtools merge -i $analysis_dir/$chr"_identityMapped_sorted.bed" > $analysis_dir/$chr"_identityMapped_sorted_merged.bed"

# C. Compute the number of base pair within each region
awk '{ $4 = $3 - $2} 1' $analysis_dir/$chr"_toMaskRegions_sorted_merged.bed" > $analysis_dir/$chr"_toMaskRegions_sorted_merged_diff.bed"

awk '{ $4 = $3 - $2} 1' $analysis_dir/$chr"_identityMapped_sorted_merged.bed" > $analysis_dir/$chr"_identityMapped_sorted_merged_diff.bed"

toMaskSum=`awk '{ sum += $4 } END {print sum} ' $analysis_dir/$chr"_toMaskRegions_sorted_merged_diff.bed"`
identityMappedSum=`awk '{ sum += $4 } END {print sum} ' $analysis_dir/$chr"_identityMapped_sorted_merged_diff.bed"`
